/* ============================================
   ...
   ============================================
   Renders the activity log table from the real backend
   (GET /api/activity-logs). Real rows use user_name/role/created_at,
   not the old demo fixture's user/timestamp field names.
*/
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function fetchLogs() {
    allLogs = (await window.RCA_API.getActivityLogs({ limit: 500 })) || [];
  }
  ...
  function getFiltered() {
    ...
    return allLogs.filter(log => {
      ...
      if (search) {
        const hay = `${log.user_name} ${log.target} ${log.action} ${log.category}`.toLowerCase();
  ...
  function render() {
    ...
    tableBody.innerHTML = logs.map(log => {
      ...
      const initials = (log.user_name || 'SY').split(' ').map(p => p[0]).join('').substring(0, 2).toUpperCase();
      ...
      return `<tr>
        ...
        <td>
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <div>
              <div style="font-size:0.8rem;font-weight:600;color:#111827">${log.user_name || 'System'}</div>
  ...
  setInterval(async () => { await fetchLogs(); renderStats(); render(); }, 30000);
